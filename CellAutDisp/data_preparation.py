...
def create_traffic_emission_columns(df, traff_coeff=np.nan, TrI_coeff=np.nan, traffroadlength_coeff=np.nan, suffix=""):
    ...
    - traff_coeff (float, optional): Coefficient for traffic volume. Default is NaN.
    - traffroadlength_coeff (float, optional): Coefficient for the interaction between traffic volume and road length. Default is NaN.
    ...
    Example:
    >>> df = pd.DataFrame({'TrV0_1': [10, 20, 30], 'road_length': [50, 60, 70], 'baseline_NO2': [5, 5, 5], 'ON_ROAD': [1, 0, 1]})
    >>> updated_df = create_traffic_emission_columns(df, traff_coeff=2, traffroadlength_coeff=0.5)
    """
    ...
    traffIntsenshours = ["TrI0_1","TrI1_2","TrI2_3","TrI3_4","TrI4_5","TrI5_6",
    ...
    if np.isnan(traffroadlength_coeff) and np.isnan(TrI_coeff) and not np.isnan(traff_coeff):
        print("only Traffic Volume without roadlength interaction")
        df[TrafficNO2_nobaseline] = df[traffVolhours] * traff_coeff
    elif np.isnan(TrI_coeff) and not np.isnan(traff_coeff) and not np.isnan(traffroadlength_coeff):
        print("Traffic volume with roadlength interaction")
        df[TrafficNO2_nobaseline] = (df[traffVolhours] * traff_coeff) + (df['road_length'] * df[traffVolhours] * traffroadlength_coeff)
    elif not np.isnan(TrI_coeff) and not np.isnan(traff_coeff) and np.isnan(traffroadlength_coeff):
        print("with traffic intensity and traffic volume")
        df[TrafficNO2_nobaseline] = (df[traffVolhours] * traff_coeff) + (df[traffIntsenshours] * TrI_coeff)
    elif not np.isnan(TrI_coeff) and np.isnan(traff_coeff) and np.isnan(traffroadlength_coeff):
        print("only traffic intensity")
        df[TrafficNO2_nobaseline] = df[traffIntsenshours] * TrI_coeff
    ...
    df_final = df.loc[:,["baseline_NO2","ON_ROAD","road_length", TrafficNO2_nobaseline, TrafficNO2, BaselineTrafficNO2]]
